...
{
    Task<(List<Tour360RequestDto> Items, int TotalItems)> GetAllAsync(
        GetTour360RequestsRequest request
    );

    Task<Tour360RequestDto> CreateAsync(
    ...
    Task<bool> UpdateScheduledDateAsync(Guid id, long scheduledDateUnix);

    Task<long?> GetLastRequestDateNonCancelledByEnvironmentAsync(Guid environmentPublicId);

    Task<(List<Tour360RequestDto> Items, int TotalItems)> GetAllByDayAsync(
